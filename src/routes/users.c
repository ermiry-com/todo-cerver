...
void users_register_handler (
	...
	TodoUserInput input = TODO_USER_INPUT_NONE;

	char token[HTTP_JWT_TOKEN_SIZE] = { 0 };
	size_t token_size = 0;

	User *user = todo_user_register (
		request->body, &error, &input
	);

	switch (error) {
		case TODO_USER_ERROR_NONE: {
			(void) todo_user_generate_token (
		...
		case TODO_USER_ERROR_BAD_REQUEST:
			users_send_input_error (http_receive, input);
			break;

		case TODO_USER_ERROR_MISSING_VALUES:
			users_send_input_error (http_receive, input);
			break;

		case TODO_USER_ERROR_REPEATED:
			(void) http_response_send (repeated_email, http_receive);
			break;

		case TODO_USER_ERROR_NOT_FOUND:
			(void) http_response_send (user_not_found, http_receive);
			break;

		case TODO_USER_ERROR_SERVER_ERROR:
...
void users_login_handler (
	...
	switch (error) {
		...
		case TODO_USER_ERROR_WRONG_PSWD:
