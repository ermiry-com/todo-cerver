...
static void user_doc_parse (
	...
	if (bson_iter_init (&iter, user_doc)) {
		...
		while (bson_iter_next (&iter)) {
			...
			else if (!strcmp (key, "itemsCount")) {
				user->items_count = value->value.v_int32;
			}
		}
	}
...
bson_t *user_create_update_todo_items (void) {

	bson_t *doc = bson_new ();
	if (doc) {
		bson_t inc_doc = BSON_INITIALIZER;
		(void) bson_append_document_begin (doc, "$inc", -1, &inc_doc);
		(void) bson_append_int32 (&inc_doc, "itemsCount", -1, 1);
...
unsigned int user_add_items (const User *user) {

	return mongo_update_one (
		users_model,
		user_query_id (user->id),
		user_create_update_todo_items ()
	);
